...
app.get('/deleteemp',(req,res)=>{
    ...
    let output={status:false,message:"Data not deleted"};
    con.query('delete from employee where empid=?',[input],(err,rows)=>{
        if(err){
            console.log("Error"+err)
        }
        else{
            ...
            {
                output.status=true;
                output.message="Employee data deleted successfully"
                res.send(output);
            }
        }
